...
const App = (() => {
  ...
  async function findGame(sessionId) {
    ...
    try {
      ...
      if (game.pin) pinSection.style.display = 'block';
      ...
      // Firebase not ready yet — retry once after a short delay
      if (e.message && e.message.includes('not initialized')) {
        UI.showToast('Connecting to Firebase, please try again...', 'info');
        setTimeout(() => findGame(sessionId), 1500);
      } else {
        UI.showToast('Error: ' + e.message, 'error');
      }
    } finally {
  ...
  function showPlayerResult(me) {
    ...
    // Use actual balance change (prevBal saved when resolving started)
    const prevBal = state.prevBals[state.playerId];
    const delta   = prevBal != null ? me.balance - prevBal : GameLogic.calculateDelta(me.outcome, me.bet);
  ...
  // init() is async (awaits Firebase config fetch) — wrap in anonymous async
